...
/** Windows only: use GetAdaptersAddresses() function to retrieve network
 * interface addresses of current machine and return them to caller as
 * smartlist of <b>tor_addr_t</b>  structures.
 */
STATIC smartlist_t *
...
#define FLAGS (GAA_FLAG_SKIP_ANYCAST | \
  ...
  /* Cast through a generic function pointer, to silence a spurious
   * compiler warning on 64-bit Windows. This cast is safe, because
   * we are casting to the correct type for GetAdaptersAddresses(). */
