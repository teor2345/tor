...
buf_read_from_tls(buf_t *buf, tor_tls_t *tls, size_t at_most)
{
  ...
  while (at_most > total_read) {
    ...
    if (readlen > sizeof tmp)
      readlen = sizeof tmp;
    r = tor_tls_read(tls, tmp, readlen);
    if (r < 0)
      return r; /* Error */
    buf_add(buf, tmp, r);
...
/** Helper for buf_flush_to_tls(): try to write <b>sz</b> bytes from
 * <b>data</b> of buffer <b>buf</b> onto socket <b>s</b>.  On success, deduct
 * the bytes written from *<b>buf_flushlen</b>.  Return the number of bytes
 * written on success, and a TOR_TLS error code on failure or blocking.
 */
static inline int
flush_tls(tor_tls_t *tls, buf_t *buf, const char *data, size_t sz,
          size_t *buf_flushlen)
{
  int r;
  ...
  log_debug(LD_NET,"flushed %d bytes, %d ready to flush, %d remain.",
            r,(int)*buf_flushlen,(int)buf_datalen(buf));
  return r;
}
              ...
              size_t *buf_flushlen)
{
  ...
  do {
    size_t flushlen0;
    if (sz <= (ssize_t)sizeof tmp)
      flushlen0 = sz;
    else
      flushlen0 = sizeof tmp;
    tor_assert(sz <= (ssize_t)buf_datalen(buf));

    buf_peek(buf, tmp, flushlen0);
    r = flush_tls(tls, buf, tmp, flushlen0, buf_flushlen);
    if (r < 0)
      return r;
    flushed += r;
    sz -= r;
