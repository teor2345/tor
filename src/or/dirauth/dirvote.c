...
#define IF_TIME_FOR_NEXT_ACTION(when_field, can_skip, skip_field, done_field, \
                                action_str)                                   \
  if (! voting_schedule.done_field) {                                         \
    if (voting_schedule.when_field > now) {                                   \
      return voting_schedule.when_field;                                      \
    } else if (can_skip && voting_schedule.skip_field <= now) {               \
      log_notice(LD_DIR, "Too late to %s, skipping.", action_str);            \
      voting_schedule.done_field = 1;                                         \
    } else {                                                                  \
...
#define IF_TIME_FOR_NEXT_SKIPPABLE_ACTION(when_field, skip_field, done_field, \
                                          action_str)                         \
        IF_TIME_FOR_NEXT_ACTION(when_field, 1, skip_field, done_field,        \
                                action_str)

#define IF_TIME_FOR_NEXT_REQUIRED_ACTION(when_field, done_field, action_str)  \
...
#undef IF_TIME_FOR_NEXT_REQUIRED_ACTION
#undef IF_TIME_FOR_NEXT_SKIPPABLE_ACTION
#undef ENDIF
