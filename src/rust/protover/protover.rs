...
impl UnvalidatedProtoEntry {
    ...
    pub fn get_unsupported(&self) -> Self {
        ...
        for (protocol, versions) in self.iter() {
            ...
            let supported_versions = match supported.get(&supported_protocol) {
                Some(v) => v,
                None => {
                    if !versions.is_empty() {
                        unsupported.insert(protocol.clone(), versions.clone());
            ...
            let mut unsupported_versions: ProtoSet = versions.clone();
...
impl IntoIterator for ProtoverVote {
    ...
    type IntoIter =
        hash_map::IntoIter<UnknownProtocol, HashMap<Version, usize>>;
...
//
// C_RUST_COUPLED: src/rust/protover.c `compute_for_old_tor`
pub fn compute_for_old_tor(
    version: &str,
) -> Result<&'static str, ProtoverError> {
