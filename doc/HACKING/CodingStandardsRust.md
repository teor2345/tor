...
To add/update dependencies, first add/update your dependencies
in the appropriate *crate-level*
...
Then `cd src/rust/ && rm -f .cargo/config && cargo update`.
Review the changes to src/rust/Cargo.lock before proceeding, but do not
commit the changes yet.

Next, run `scripts/maint/updateRustDependencies.sh`, then go into src/ext/rust
to commit the changes with `git add --all; git commit`. Push the commit
